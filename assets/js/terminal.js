...
const GEMINI_MARK = '<svg class="oai gem-mark" viewBox="0 0 24 24" aria-hidden="true"><path d="M12 0c0 6.627 5.373 12 12 12-6.627 0-12 5.373-12 12 0-6.627-5.373-12-12-12C6.627 12 12 6.627 12 0Z"/></svg>';

/* The mark belongs to whichever model is doing the work, so a fallback from
   one provider to the other is visible while it is happening, not only
   afterwards in the header. */
function providerMark() {
  const p = (typeof ProviderLights !== 'undefined' && ProviderLights.primary) || 'groq';
  return p === 'gemini' ? GEMINI_MARK : OAI_MARK;
}
...
const Spinner = {
  ...
  start(body) {
    ...
    body.innerHTML = '<span class="spin">' + providerMark() +
